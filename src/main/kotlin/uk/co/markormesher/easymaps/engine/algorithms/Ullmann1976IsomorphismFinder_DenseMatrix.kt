...
import uk.co.markormesher.easymaps.engine.structures.DenseBooleanMatrix
...
class Ullmann1976IsomorphismFinder_DenseMatrix(candidate: Network, master: Network): IsomorphismFinder(candidate, master) {
	...
	private fun calcInitialPossibleAssignments(): DenseBooleanMatrix {
		val m = DenseBooleanMatrix(master.nodeCount, candidate.nodeCount)
	...
	private fun search(assignment: MutableMap<Int, Int>, possibleAssignments: DenseBooleanMatrix, lastAssigned: Int = -1) {
		...
		possibleAssignments.forEachOnRow(pivotNode, { possibleMNode, edge ->
			if (!edge || assignment.containsValue(possibleMNode)) return@forEachOnRow
	...
	private fun prunePossibleAssignments(possibleAssignments: DenseBooleanMatrix) {
		var changes = true
		while (changes) {
			changes = false
			...
			for (cNode in 0..candidate.nodeCount - 1) {
				...
				possibleAssignments.forEachOnRow(cNode, { mNode, edge ->
					if (!edge) return@forEachOnRow
